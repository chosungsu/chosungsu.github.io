...
1. Goal-Conditioned Reinforcement Learning (GCRL)
...
2. Bridge Planning and Single-Agent RL

To solve Long-Horizon Tasks, there have been experiments combining RL with single-agent planning, proposing Search on Replay Buffer (SoRB). In this environment, R($s, a, s_g$) = -1 and γ = 1 are defined, and the goal-conditioned policy π($a∣s,s_g$) and its corresponding value function V($s, s_g$) are learned using standard off-policy RL.
...
3. Constrained Reinforcement Learning (CRL)
...
1. Goal-Conditioned Safe Reinforcement Learning
...
4. Graph Construction from Replay Buffer
...
5. Collision-Based Search (CBS) for Multi-Agent Path Finding
...
Experiment Overview and Evaluation Questions
