...
<img src="https://velog.velcdn.com/images/devjo/post/19a2950e-8e9d-4935-996c-1a0b56d41894/image.png" alt="Example Image" style="display: block; margin: 0 auto; height:150;" />

The Delaunay triangulation is defined as the dual structure of the Voronoi diagram for a given set of points. For each cell (site) in the Voronoi diagram, a vertex is created. For every Voronoi edge between two sites $p_i$ and $p_j$, an edge is drawn between $p_i$ and $p_j$ in the Delaunay triangulation. Each vertex in the Voronoi diagram corresponds to a face in the dual graph.

The outer face of the Delaunay triangulation coincides with the boundary of the convex hull of the point set.

#### Minimum Spanning Tree, MST

Given $n$ points, we can construct a Euclidean graph by connecting every pair of points with an edge weighted by their Euclidean distance.

The fastest way to compute the MST is to first construct the Delaunay triangulation in $O(n \log n)$ time, and then run Kruskal’s algorithm on this graph. The total running time is $O(n \log n)$.

#### Spanner Properties

The edges of the Delaunay triangulation form a reasonable transportation network among the points. For a graph $G$, if the shortest path distance $\delta_G(p, q)$ between any two points $p, q$ does not exceed $t$ times the Euclidean distance $|pq|$, we call $G$ a $t$-spanner.
