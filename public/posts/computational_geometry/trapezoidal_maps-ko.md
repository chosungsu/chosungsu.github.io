...
들로네 삼각 분할은 점 집합의 보로노이 다이어그램(Voronoi Diagram)의 쌍대(dual) 구조로 정의됩니다. 보로노이 다이어그램의 각 셀(site)에 대해 정점을 생성하고 두 사이트 $p_i$와 $p_j$ 사이의 보로노이 모서리에 대해, $p_i$와 $p_j$를 연결하는 모서리(edge)를 생성합니다. 보로노이 다이어그램의 각 정점은 쌍대 그래프의 면(face)에 해당합니다.

들로네 삼각 분할의 외부 면의 경계는 점 집합의 볼록 껍질의 경계입니다.

#### Minimum Spanning Tree, MST

$n$개의 점이 주어졌을 때, 모든 점 쌍을 모서리로 하고 그 유클리드 거리를 가중치로 하는 유클리드 그래프를 생각할 수 있습니다.

MST를 계산하는 가장 빠른 방법은 $O(n \log n)$ 시간에 들로네 삼각 분할을 계산한 다음, 그 들로네 삼각 분할을 그래프로 간주하여 크러스컬 알고리즘을 적용하는 것입니다. 이 총 실행 시간은 $O(n \log n)$입니다.

#### Spanner Properties

들로네 삼각 분할의 모서리는 점들 사이의 합리적인 수송 도로망을 형성합니다. 그래프 $G$에서 임의의 두 점 $p, q$ 사이의 최단 경로 길이 $\delta_G(p, q)$가 유클리드 거리 $|pq|$의 $t$배를 넘지 않을 때, $G$를 $t$-스패너라고 합니다.
